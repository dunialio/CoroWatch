...
static void battery_callback(BatteryChargeState state){
  if(state.is_charging){
    static char battery_text[] = "Ngisi 100%";
    layer_set_hidden(text_layer_get_layer(charging_layer), false);
    snprintf(battery_text, sizeof(battery_text), "Ngisi %d%%", state.charge_percent);
    text_layer_set_text(charging_layer, battery_text);
  }
  else if(state.is_plugged){
    layer_set_hidden(text_layer_get_layer(charging_layer), false);
    text_layer_set_text(charging_layer, "Kebek");
  }
  else {
...
static void show_detail_face(){
  ...
  //goyang gawe dilok batre
  BatteryChargeState charge_state = battery_state_service_peek();
  static char battery_text[] = "100%";
  snprintf(battery_text, sizeof(battery_text), "%d%%", charge_state.charge_percent);
  text_layer_set_text(battery_layer, battery_text);
  
  //sumet tanggal pisan nek pas goyang
  static const char* const DAYS[] = {"Minggu", "Senen", "Seloso", "Rebo", "Kemis", "Jemuah", "Septu" };
...
static void handle_second_tick(struct tm* tick_time, TimeUnits units_changed) {
  static char current_hour[] = "rolas";
  static char current_tens[] = "rolas";
  static char current_ones[] = "rolas";
